...
public class HelicallyScan2 : MonoBehaviour
{
    ...
    private float screenshotInterval = 0.5f;
    ...
    private float initialZ = -2.2f;
    private float currentZ = -2.2f;
    private float scanningZ = -2.6f;
    private float targetZ = -3.0f;
    private float zStep = 0.3f;
    private float fixedX = 0.64f;
    private float fixedY = 9.1f;
    private float initialTiltAngle = 30f;
    private float finalTiltAngle = 150f;

    
    private float rotationAngle = 0f; // Current rotation angle
    private float rotationStep = 30f;  // Rotate 2 degrees per step
    private float tiltStep = 30f;
    ...
    */
    ...
    IEnumerator HelicalScan()
    {
        ...
        while (isAutoRotating && currentZ >= scanningZ)
        {
        ...
        // Final scan at (fixedX, fixedY, scanningZ)
        Debug.Log("Starting scanningZ tilt scan");
        transform.position = new Vector3(fixedX, fixedY, scanningZ);
        ...
        // Helical scan: Move helically from scanningZ to targetZ
        while (isAutoRotating && currentZ >= targetZ)
        {
            ...
            // Set rotation: fixed tilt (150 degrees) and rotate around the Z-axis
            Quaternion tilt = Quaternion.Euler(finalTiltAngle, 0, 0); // Tilt by 150 degrees
            Quaternion rotation = Quaternion.Euler(0, 0, rotationAngle);
            transform.rotation = rotation * tilt;
