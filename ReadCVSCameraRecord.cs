...
public class ReadCVSCameraRecord : MonoBehaviour
{
    ...
    private List<CameraTransformData> cameraDataList; // List to store parsed data from CSV
    void Start()
    {
        ...
        SaveParameters();
        // Load data from CSV when the scene starts
        LoadCameraDataFromCSV();
    ...
    // Class to hold one row of CSV data
    private class CameraTransformData
    {
        public Vector3 Position { get; set; }
        public Quaternion Rotation { get; set; }
        public float Time { get; set; } // Time in seconds
    }    
    private void LoadCameraDataFromCSV()
    {
        cameraDataList = new List<CameraTransformData>();
	string CVSDirectory="Recordings/2025-03-31-09-34/Camera_position_rotation.csv";
        string filePath =CVSDirectory;
        ...
        {
            string headerLine = sr.ReadLine(); // Skip the header line (tX,tY,tZ,rX,rY,rZ,rW,time(s))
            while (!sr.EndOfStream)
            {
                string line = sr.ReadLine();
                string[] values = line.Split(',');

                if (values.Length == 8) // Ensure the row has all the required data
                {
                    float tX = float.Parse(values[0]);
                    float tY = float.Parse(values[1]);
                    float tZ = float.Parse(values[2]);

                    float rX = float.Parse(values[3]);
                    float rY = float.Parse(values[4]);
                    float rZ = float.Parse(values[5]);
                    float rW = float.Parse(values[6]);

                    float time = float.Parse(values[7]);

                    cameraDataList.Add(new CameraTransformData
                    {
        ...
        Debug.Log($"Loaded {cameraDataList.Count} rows from CSV.");
    }
    void SaveParameters()
    {
    ...
    */
    public IEnumerator AutoRotateCamera()
    {
        ...
        {
            Debug.LogError("No camera data loaded. Cannot start auto-rotation.");
            yield break;
        }
        ...
        foreach (var data in cameraDataList)
        {
            ...
            // Debug: Log the data being applied
            Debug.Log($"Applying Position: {data.Position}, Rotation: {data.Rotation.eulerAngles}, Time: {data.Time}");

            // Update camera position and rotation
            transform.position = data.Position;
            transform.rotation = data.Rotation;

            // Wait for 2 seconds before moving to the next data point
            yield return new WaitForSeconds(2.0f);

            // Optionally, capture frames or record transforms here
            CaptureFrames();
            RecordTransform();
        }
